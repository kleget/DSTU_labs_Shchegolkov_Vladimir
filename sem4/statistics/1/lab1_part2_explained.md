...
Все вычисления сделаны через библиотеку `NumPy`.
...
1. `np.array(...)` превращает входные данные в массив NumPy.
2. `np.unique(data, return_counts=True)` сразу дает `xi` и `ni`.
3. `pi = ni / n` считает относительные частоты.
4. `np.cumsum(ni)` строит накопленные частоты `Ni`.
5. `Fi = Ni / n` считает накопленные относительные частоты.
...
```mermaid
flowchart TD
    A[Входные данные] --> B[np.unique: xi и ni]
    B --> C[Сортировка уникальных xi]
    C --> D[Расчет ni и pi]
    D --> E[Накопление Ni через cumsum]
    E --> F[Расчет Fi]
